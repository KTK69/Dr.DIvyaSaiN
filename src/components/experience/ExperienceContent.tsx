...
export default function ExperienceContent() {
  ...
  const { experience, publications, research, conferences, awards } = content.experience;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 section-padding space-y-20">
        ...
        <section aria-labelledby="awards-exp-heading">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 id="awards-exp-heading" className="text-2xl font-medium text-(--foreground)" style={{ fontFamily: "var(--font-serif)" }}>
              Awards &amp; Honours
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {awards.map((award) => (
              <div key={award.title} className="glass-card rounded-xl p-6 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-20 h-20 rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(184,151,42,0.08) 0%, transparent 70%)" }} />
                <p className="text-3xl font-medium text-(--accent-gold-light) mb-3" style={{ fontFamily: "var(--font-serif)" }}>{award.year}</p>
